...
router.post('/create', async (req, res) => {
    try {
        ...
        const vault = await vaultService_1.vaultService.createVault(request);
...
router.get('/:id', async (req, res) => {
    try {
        const vault = await vaultService_1.vaultService.getVault(req.params.id);
        if (!vault) {
...
router.post('/:id/deposits', async (req, res) => {
    try {
        ...
        const vault = await vaultService_1.vaultService.updateDeposits(req.params.id, amount);
        if (!vault) {
            return res.status(404).json({ error: 'Vault not found' });
        }
        res.json(vault);
...
router.post('/:id/approve', async (req, res) => {
    try {
        ...
        const vault = await vaultService_1.vaultService.approveUnlock(req.params.id, holderId);
        if (!vault) {
            return res.status(404).json({ error: 'Vault not found' });
        }
        res.json(vault);
...
router.post('/:id/proposal', async (req, res) => {
    try {
        ...
        const proposal = await proposalService_1.proposalService.createProposal(req.params.id, request);
        if (!proposal) {
            return res.status(404).json({ error: 'Vault not found' });
        }
...
router.get('/:id/proposal', async (req, res) => {
    try {
        const proposal = await proposalService_1.proposalService.getProposalByVaultId(req.params.id);
        if (!proposal) {
...
// Approve proposal
router.post('/:id/proposal/approve', async (req, res) => {
    try {
        const { holderId } = req.body;
        if (!holderId) {
            return res.status(400).json({ error: 'Holder ID required' });
        ...
        const updated = await proposalService_1.proposalService.approveProposal(proposal.id, holderId);
        res.json(updated);
    }
    catch (error) {
...
// Execute proposal
router.post('/:id/proposal/execute', async (req, res) => {
    try {
        const proposal = await proposalService_1.proposalService.getProposalByVaultId(req.params.id);
        if (!proposal) {
            return res.status(404).json({ error: 'Proposal not found' });
        }
        const success = await proposalService_1.proposalService.executeProposal(proposal.id);
        if (!success) {
...
router.delete('/:id', async (req, res) => {
    try {
        const success = await vaultService_1.vaultService.destroyVault(req.params.id);
        if (!success) {
            return res.status(404).json({ error: 'Vault not found' });
        }
